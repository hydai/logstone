...
import * as mountSelectors from '../lib/lodestone-css-selectors/profile/mount.json';
...
export class MountParser {
  parse(html: string): MountsData {
    const document = parseDocument(html);
    const mounts: MountData[] = [];

    // 解析坐騎列表
    if (mountSelectors.MOUNTS?.ROOT) {
      const elements = this.querySelectorAll(document, mountSelectors.MOUNTS.ROOT.selector);
      
      for (const element of elements) {
        const mount = this.parseMountEntry(element);
        if (mount) {
          mounts.push(mount);
        }
      }
    ...
    // 解析總數
    let total = mounts.length;
    if (mountSelectors.TOTAL) {
      const totalElement = this.querySelector(document, mountSelectors.TOTAL.selector);
      if (totalElement) {
  ...
  private parseMountEntry(element: Element): MountData | null {
    ...
    // 解析圖標
    if (mountSelectors.MOUNTS.ICON) {
      const iconElement = this.querySelector(element, mountSelectors.MOUNTS.ICON.selector);
      if (iconElement) {
        mount.Icon = this.getAttribute(iconElement, mountSelectors.MOUNTS.ICON.attribute || 'src') || '';
      }
    }

    // 確保有基本資料才返回
    if (mount.Name && mount.Icon) {
      return mount as MountData;
    }
